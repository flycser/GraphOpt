...
def test():
    ...
    normalize = True
    for lmbd in [0.07]:
    # for lmbd in [0.0, 0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.06, 0.07, 0.08, 0.09]:
    ...
    # for lmbd in np.arange(0.31, 0.51, 0.01):
        ...
        # for ratio in np.arange(1., 4., 1.):
            for i, instance in enumerate(dataset):
                ...
                subgraph = nx.subgraph(graph, set(np.nonzero(x)[0]))

                print('performance', prec, rec, fm, iou, nx.density(graph), nx.density(subgraph), subgraph.number_of_nodes(), subgraph.number_of_edges())

                results.append((prec, rec, fm, iou, nx.density(graph), nx.density(subgraph), subgraph.number_of_nodes(), subgraph.number_of_edges()))
