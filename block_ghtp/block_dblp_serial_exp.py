...
# @Filename : block_dblp_serial_exp
# @Date     : 07/20/2019 16:03:16
# @Poject   : GraphOpt
...
def run_dataset(paras):
    dataset, sparsity, trade_off, learning_rate, max_iter, epsilon, write_to_dir, normalize = paras

    if not write_to_dir:
        logger = logging.getLogger('fei')
    else:
        log_fn = '{}_sparsity_{:d}_trade_{}_lr_{}_{}_serial.txt'.format(DATASET, sparsity, trade_off, learning_rate, normalize)
...
def train_mps(sparsity, trade_off, learning_rate, normalize):
    ...
    epsilon = 1e-3
    # write_to_dir = None
    write_to_dir = '/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_dblp'
...
if __name__ == '__main__':
    ...
    sparsity, trade_off, learning_rate, normalize = int(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]), sys.argv[4].lower() == 'true'
