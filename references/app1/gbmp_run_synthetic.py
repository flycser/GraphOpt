...
def dynamic_graph_mp(data, k, max_iter, trade_off, log_file, func_name="EMS"):
    ...
    if func_name == "SumEMS":
        ...
        func = EMS.SumEMS(feature_matrix=features, trade_off=trade_off)
    ...
    for iter in range(max_iter):
        ...
        global_prec, global_rec, global_fm, global_iou, valid_prec, valid_rec, valid_fm, valid_iou = evaluate(true_subgraphs, pred_subgraphs)
...
def worker(para):
    ...
    global_prec, global_rec, global_fm, global_iou, valid_prec, valid_rec, valid_fm, valid_iou = evaluate(true_subgraphs, raw_pred_subgraphs, log_file)
    if func_name == "SumEMS":
        obj_val, nonoverlap_penalty = func.get_obj_value(XT)
        ems_score = func.get_sum_ems(XT)
    elif func_name == "GlobalEMS":
    ...
    for t in range(T):
        ...
        refined_XT.append(x)
    global_prec, global_rec, global_fm, global_iou, valid_prec, valid_rec, valid_fm, valid_iou = evaluate(true_subgraphs, refined_pred_subgraphs, log_file)
...
def single_test():
    ...
    data_dir = "{}/nodes_{}_windows_{}_mu_{}_subsize_{}_{}_range_{}_{}_overlap_{}_{}_{}.pkl".format(
        graph_type, num_nodes, num_time_slots, mu1, subsize_min, subsize_max, start_time, end_time, overlap, pattern, data_type)
    print(data_dir)
