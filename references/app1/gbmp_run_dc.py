...
def dynamic_graph_mp(data, sparsity, max_iter, trade_off, log_file, func_name="EMS"):
    """
    ...
    num_events = 10
    top_k_true_subgraphs = data["top_k_true_subgraphs"][:num_events]
    ...
    for iter in range(max_iter):
        ...
        for t in range(T):
            ...
            re_head = head_proj(edges=edges, weights=costs, x=normalized_grad, g=20, s=sparsity, budget=sparsity - 1, delta=1. / 169., max_iter=100,
        ...
        for t in range(T):
            bx = BX[t]
            re_tail = tail_proj(edges=edges, weights=costs, x=bx, g=20, s=sparsity, budget=sparsity - 1, nu=2.5, max_iter=100, err_tol=1e-8, root=-1,
...
def worker(para):
    (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file, num_events) = para
    ...
    if func_name == "SumEMS":
        func = EMS.SumEMS(feature_matrix=data["features"], trade_off=trade_off)
    ...
    print('trade off {}, sparsity {}, case {}, precision {:.5f}, recall {:.5f}, f-measure {:.5f}\n'.format(
...
def single_test():
    ...
    week = 2
    weekday = 0
    case_id = week * 5 + weekday
    data_dir = "{}/week_{}_weekday_{}.pkl".format(
        graph_type, week, weekday)
    ...
    for trade_off in [x * 0.01 for x in range(1, 2)]:
        for sparsity in [200]:
            # print trade_off, sparsity
            # log_file = "../logs/{}/run{}/{}/week_{}_weekday_{}_tradeoff_{}_sparsity_{}.out".format(
            #     graph_type, run, func_name, week, weekday, trade_off, sparsity)
            ...
            para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file, num_events)
