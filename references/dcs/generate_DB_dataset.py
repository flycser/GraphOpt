...
# @Filename : generate_DB_dataset
# @Date     : 07/31/2019 22:34:56
# @Poject   : GraphOpt
...
def extract_info_file():
    ...
    dm_confs = set(('SIGMOD', 'VLDB', 'ICDE'))

    count = 0
    authors = set()
    x = []
    with open(os.path.join(path, fn)) as rfile, open(os.path.join(path, 'db.txt'), 'w') as wfile:
        while True:
            record = []
            line = rfile.readline()
            tag = False
...
def convert_id_from_zero():
    ...
    fn = 'db_author_map.pkl' # old id, new id
    with open(os.path.join(path, fn), 'wb') as wfile:
    ...
    author_term_mat = np.zeros((relabeled_dm_first_graph.number_of_nodes(), len(author_term_dict[3])))

    print(len(author_term_dict[3]))
...
def calculate_coefficient():
    ...
    fn = 'db_correlations.pkl'
    with open(os.path.join(path, fn), 'wb') as wfile:
        pickle.dump(correlations, wfile)


def min_max_common_entries(): # 134 0 0.5331347910376405
    path = '/network/rit/lab/ceashpc/share_data/GraphOpt/datasets/DBLP/DBLP_Citation_2014_May/DB'
...
def count_papers():
    ...
    print(len(author_old_new_map))

    print(db_authors)
    print(len(db_authors))

    weight = np.zeros(len(author_old_new_map))
                    ...
                    if line.startswith('#@'):
                        ...
                        for author in paper_authors:
                            ...
                            if db_authors[author] not in author_old_new_map.keys():
                                ...
                                weight[author_old_new_map[db_authors[author]]]
...
def construct_dual_network():
    path = '/network/rit/lab/ceashpc/share_data/GraphOpt/datasets/DBLP/DBLP_Citation_2014_May/DB'
    fn = 'db_correlations.pkl'
    with open(os.path.join(path, fn), 'wb') as rfile:
...
def relabeled_first_graph():
    ...
    fn = 'db_relabeled_first_graph.pkl'
    with open(os.path.join(path, fn), 'wb') as wfile:
...
def generate_datasets():
    ...
    fn = 'db_author_paper_count.pkl' # db_author_paper_count include author not in graph, which are not counted, their values are zeros
    with open(os.path.join(path, fn), 'rb') as rfile:
    ...
    fn = 'db_top{}_dataset.pkl'.format(topk)
    with open(os.path.join(path, fn), 'wb') as wfile:
...
def generate_db_dcs():
    rpath = '/network/rit/lab/ceashpc/share_data/GraphOpt/datasets/DBLP/DBLP_Citation_2014_May/DB'
    fn = 'db_top30000_dataset.pkl'
    ...
    wpath = '/network/rit/lab/ceashpc/share_data/GraphOpt/datasets/DBLP/DBLP_Citation_2014_May/dcs_db'
