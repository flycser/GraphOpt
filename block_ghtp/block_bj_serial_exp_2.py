...
def run_case():
    ...
    for case_id in range(10):
        data_dir = "/network/rit/lab/ceashpc/share_data/{}/run{}/pkl/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_deg_{}_{}_{}.pkl".format(
        ...
        for trade_off in [x * 0.01 for x in range(1, 2)]:
            for sparsity in spar_list:
                ...
                #     f.write("trade_off= {}, sparsity={}\n".format(trade_off, sparsity))
                log_file = None
                result_file = "/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_ba_4/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_deg_{}_{}_serial.out".format(num_nodes, num_blocks, mu1, true_size, true_size, deg, data_type)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                input_paras.append(para)
                worker(para)
...
def run_dblp():
    ...
    for case_id in range(7, 8):
        if percent > 0:
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_{}-{}_{}_{}_{}_False_{}p_{}_new.pkl'.format(start_year, end_year, num_std, num_blocks, subsize, percent, case_id)
        # else:
        ...
        if normalized:
            # data['features'][data['features'] > 10] = 10.
            data['features'] = data['features'] / np.max(data['features'])
            # normalized = not normalized
            # print(np.max(data['features']))
        ...
        for trade_off in [x * 0.0005 for x in range(1, 2)]:
            for sparsity in spar_list:
                ...
                #     f.write("trade_off= {}, sparsity={}\n".format(trade_off, sparsity))
                log_file = None
                result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp_new/serial_{}_{}_{}_{}_{}_{}_{}_m2std_{}_new.out'.format(
...
def run_bj():
    ...
    with open('/network/rit/lab/ceashpc/share_data/GraphOpt/app2/bj/train.pkl', 'rb') as rfile:
        dataset = pickle.load(rfile)

    for case_id in range(len(dataset)):
        # if percent > 0:
        #     data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_{}-{}_{}_{}_{}_False_{}p_{}_new.pkl'.format(
        #         start_year, end_year, num_std, num_blocks, subsize, percent, case_id)
        # else:
        #     data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_1995-2005_{}_{}_{}_{}.pkl'.format(
        #         num_std, num_blocks, subsize, normalized)
        ...
        for trade_off in [x * 0.0005 for x in range(1, 2)]:
            for sparsity in spar_list:
                ...
                #     f.write("trade_off= {}, sparsity={}\n".format(trade_off, sparsity))
                log_file = None
                result_file = '/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_bj_2/serial_{}.out'.format(case_id)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                input_paras.append(para)
                worker(para)
