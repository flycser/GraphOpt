...
# @Filename : block_bj_parallel_exp_2
# @Date     : 07/28/2019 01:19:50
# @Poject   : GraphOpt
...
def single_test():
    ...
    func_name = "PartitionEMS"
    max_iter = 10
    true_size = num_nodes * 0.05
    data_type = "train"
    mu1 = 5
    deg = 10

    case_id = 1
    data_dir = "/network/rit/lab/ceashpc/share_data/{}/run{}/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_deg_{}_{}_{}.pkl".format(
        graph_type, run, num_nodes, num_blocks, mu1, true_size, true_size, deg, data_type, case_id)
    all_data = pickle.load(open(data_dir, "rb"))
    data = all_data[case_id]
    true_subgraph = data["true_subgraph"]
    print("true_subgraph", true_subgraph)

    # print(type(data['nodes_set'])) # verify that nodes_set is a list
    ...
    for trade_off in [x * 0.01 for x in range(1, 2)]:
        for sparsity in spar_list:
            ...
            #     f.write("trade_off= {}, sparsity={}\n".format(trade_off, sparsity))
            log_file = None
            result_file = "/network/rit/lab/ceashpc/share_data/fei_results/tmp/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_{}.out".format(
                graph_type, run, func_name, num_nodes, num_blocks, mu1, true_size, true_size, data_type)
            para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
            input_paras.append(para)
            worker(para)
...
def run_case():
    ...
    for case_id in range(1):
        ...
        true_subgraph = data["true_subgraph"]
        print("true_subgraph", true_subgraph)
        for trade_off in [x * 0.0 for x in range(1, 2)]:
            for sparsity in spar_list:
                log_file = None
                result_file = "/network/rit/lab/ceashpc/share_data/{}/run{}/results_4/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_deg_{}_{}_parallel.out".format(graph_type, run, num_nodes, num_blocks, mu1, true_size, true_size, deg, data_type)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                input_paras.append(para)
                worker(para)
...
def run_tmp():
    ...
    for case_id in range(1):
        # data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/dataset_{}-{}.pkl'.format(start_year, end_year)
        # data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/1std/pkl/dataset_1995-2005_{}_{}_{}_{}.pkl'.format(num_std, num_blocks, subsize, normalized)
        # data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/20130912_hour_17_ind_0_normalized.pkl'
        # data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_1995-2005_{}_{}_{}_{}.pkl'.format(num_std, num_blocks, subsize, normalized)
        if percent > 0:
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_1995-2005_{}_{}_{}_{}_{}p.pkl'.format(num_std, num_blocks, subsize, normalized, percent)
        else:
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_1995-2005_{}_{}_{}_{}.pkl'.format(num_std, num_blocks, subsize, normalized)
        ...
        if not normalized:
            # data['features'] = (mean_features - np.min(mean_features)) / np.max(mean_features) # todo !!!
            data['features'] = data['features'] / np.max(data['features']) # todo !!!
            # data['features'] = (data['features'] - np.min(data['features'])) / np.max(data['features']) # todo !!!
            # data['features'] = (data['features'] - np.min(data['features'])) / (np.max(data['features']) - np.min(data['features']))

        # data = all_data[case_id]
        true_subgraph = data['true_subgraph']
        print("true_subgraph", true_subgraph)
        for trade_off in [x * 0.0005 for x in range(1, 2)]:
            for sparsity in spar_list:
                log_file = None
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/parallel_{}_{}_{}_{}_{}_m2std.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                input_paras.append(para)
                worker(para)
...
def run_wiki():
    ...
    for case_id in range(10):
        data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/wiki/blocks_20_mu_5_true_1000_case_{}.pkl'.format(case_id)
        data = pickle.load(open(data_dir, 'rb'))
        # data = all_data[case_id]
        true_subgraph = data['true_subgraph']
        print("true_subgraph", true_subgraph)
        for trade_off in [x * 0.0005 for x in range(1, 2)]:
            for sparsity in spar_list:
                log_file = None
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/wiki_{}_{}_{}.out'.format(sparsity, num_blocks, normalized)
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                input_paras.append(para)
                worker(para)
...
def run_condmat():
    ...
    for case_id in range(10):
        data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/condmat/blocks_20_mu_5_true_1000_case_{}.pkl'.format(case_id)
        data = pickle.load(open(data_dir, 'rb'))
        # data = all_data[case_id]
        true_subgraph = data['true_subgraph']
        print("true_subgraph", true_subgraph)
        for trade_off in [x * 0.001 for x in range(1, 2)]:
            for sparsity in spar_list:
                log_file = None
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/condmat_{}_{}_{}.out'.format(sparsity,
                                                                                                      num_blocks,
                                                                                                      normalized)
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
...
def run_beijing():
    ...
    for hour in hour_list:
        for case_id in range(6):
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/beijing/{}_hour_{}_ind_{}.pkl'.format(date, hour, case_id)
            data = pickle.load(open(data_dir, 'rb'))

            # if hour == 17 and case_id == 0:
            # print(len(data['true_subgraph']))

            # data = all_data[case_id]
            true_subgraph = data['true_subgraph']
            print("true_subgraph", true_subgraph)
            for trade_off in [x * 0.0001 for x in range(1, 2)]:
                for sparsity in spar_list:
                    log_file = None
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                    result_file = '/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_bj_2/parallel_{}_{}.out'.format(sparsity, trade_off)
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                    para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                    input_paras.append(para)
                    worker(para)
...
def run_node():
    ...
    for idx in range(1, 11):
        for case_id in range(2):
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/node/nodes_{}_blocks_10_mu_5_subsize_{:.1f}_{:.1f}_deg_3_train_{}.pkl'.format(1000*idx, 100.0*idx, 100.0*idx, case_id)
            data = pickle.load(open(data_dir, 'rb'))
            # data = all_data[case_id]
            spar_list = [100 * idx / 2]
            true_subgraph = data['true_subgraph']
            ...
            for trade_off in [x * 0.0005 for x in range(1, 2)]:
                for sparsity in spar_list:
                    log_file = None
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                    result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/node_{}_{}_{}.out'.format(sparsity, num_blocks, normalized)
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                    para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                    input_paras.append(para)
                    worker(para)
...
def run_edge():
    ...
    for idx in range(9, 10):
        deg = idx
        for case_id in range(2):
            # data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/edge/nodes_100000_blocks_100_mu_5_subsize_5000.0_5000.0_deg_{}_train_{}.pkl'.format(idx, case_id)
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/edge/nodes_100000_blocks_100_mu_5_subsize_5000.0_5000.0_deg_{}_train_{}.pkl'.format(idx, case_id)
            # data_dir = '/network/rit/lab/ceashpc/share_data/BA/run3/nodes_100000_blocks_100_mu_5_subsize_5000.0_5000.0_deg_{}_train_{}.pkl'.format(idx, case_id)
            data = pickle.load(open(data_dir, 'rb'))
            # print(type(data))
            # print(data.keys())
            # print(len(data))
            # data = data[0]
            data['features'] = data['features'] / np.max(data['features'])
            # data = all_data[case_id]
            spar_list = [500]
            true_subgraph = data['true_subgraph']
            print("true_subgraph", true_subgraph)
            for trade_off in [x * 0.001 for x in range(1, 2)]:
                for sparsity in spar_list:
                    log_file = None
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/edge_{}_{}_{}.out'.format(sparsity, num_blocks, normalized)
                    result_file = '/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_ba_4/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_deg_{}_{}_parallel.out'.format(num_nodes, num_blocks, mu1, true_size, true_size, deg, data_type)
                    # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                    para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                    input_paras.append(para)
                    worker(para)
...
def run_dblp():
    ...
    for case_id in range(6, 7):
        if percent > 0:
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_1995-2005_{}_{}_{}_{}_{}p_{}_new.pkl'.format(
                num_std, num_blocks, subsize, normalized, percent, case_id)
        else:
            data_dir = '/network/rit/lab/ceashpc/share_data/DBLP/m2std/pkl/dataset_1995-2005_{}_{}_{}_{}.pkl'.format(
                num_std, num_blocks, subsize, normalized)
        data = pickle.load(open(data_dir, 'rb'))
        ...
        for trade_off in [x * 0.0005 for x in range(1, 2)]:
            for sparsity in spar_list:
                log_file = None
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/1std/gbmp/dataset_{}-{}_parallel_{}_{}_{}_{}_{}_2.out'.format(start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized)
                result_file = '/network/rit/lab/ceashpc/share_data/DBLP/tmp/parallel_{}_{}_{}_{}_{}_{}_{}_m2std_{}_new.out'.format(
                    start_year, end_year, sparsity, num_std, num_blocks, subsize, normalized, case_id)
                # result_file = '/network/rit/lab/ceashpc/share_data/DBLP/dblp_coauthor/1std/gbmp/dataset_{}-{}_parallel.out'.format(start_year, end_year)
                para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
                input_paras.append(para)
                worker(para)
