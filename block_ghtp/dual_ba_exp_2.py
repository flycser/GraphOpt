...
# @Filename : dual_ba_exp_2
# @Date     : 08/04/2019 09:36:33
# @Poject   : GraphOpt
...
def run_dataset(paras):
    ...
    for i, instance in enumerate(dataset):
        ...
        logger.debug('refined x density second graph: {:.5f}'.format(nx.density(nx.subgraph(second_graph, refined_pred_subgraph_x))))
        ...
        logger.debug('refined combined density second graph: {:.5f}'.format(
            nx.density(nx.subgraph(second_graph, refined_combine_pred_subgraph))))
