...
# @Filename : block_ba_parallel_exp
# @Date     : 07/17/2019 21:26:30
# @Poject   : GraphOpt
...
def run_dataset(paras):
    ...
    if not write_to_dir:
        ...
        log_fn = '{}_nodes_{}_deg_{}_sparsity_{:d}_trade_{}_lr_{}_{}_parallel.txt'.format(DATASET, num_nodes, deg, sparsity, trade_off, learning_rate, normalize)
...
def train_mps(sparsity, trade_off, learning_rate, tao, num_nodes, deg, normalize):
    ...
    # sparsity_list = [200]
    # trade_off_list = [0.0001]
    # learning_rate_list = [1.]
    max_iter = 10
    epsilon = 1e-3
    # write_to_dir = None
    write_to_dir = '/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_ba_5'

    paras = dataset, sparsity, trade_off, learning_rate, max_iter, epsilon, write_to_dir, tao, num_nodes, deg, normalize
    run_dataset(paras)
...
if __name__ == '__main__':
    sparsity, trade_off, learning_rate, tao, num_nodes, deg, normalize = int(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5]), int(sys.argv[6]), sys.argv[7].lower() == 'true'
    train_mps(sparsity, trade_off, learning_rate, tao, num_nodes, deg, normalize)