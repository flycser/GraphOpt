...
# @Filename : PartitionEMS
# @Date     : 07/27/2019 20:49:36
# @Poject   : GraphOpt
...
class PartitionEMS(object):
    ...
    def __init__(self, features=None, num_blocks=None, nodes_set=None, boundary_edges_dict=None, nodes_id_dict=None,
                 trade_off=10.):
        """
    ...
    def get_sum_ems(self, X):
        ...
        for t in range(self.num_blocks):
            xt = X[sorted(self.nodes_set[t])] # get x of current block
            fea = self.features[sorted(self.nodes_set[t])] # get features of current block
    ...
    def get_argmax_fx_with_proj(self, XT, Omega_X):
        ...
        for i in range(max_iter):
            ...
            for t in range(self.num_blocks):
                ...
                boundary_xs_dict = self.get_boundary_xs(X, self.boundary_edges_dict[t],
                                                        self.nodes_id_dict)  # key is boundary edge, value is adjacent x in other blocks
    ...
    def get_argmax_fx_with_proj_parallel(self, XT, Omega_X):
        ...
        for k in range(max_iter):
            ...
            for block_index in S:
                ...
                para = (block_index, theta, tao, n, self.nodes_set, self.current_X, self.current_Y, self.current_Z,
                        self.Omega_X)
    ...
    def block_worker(self, para):
        ...
        v = 1000.0
        boundary_xs_dict = self.get_boundary_xs(self.current_Y, self.boundary_edges_dict[block_index],
                                                self.nodes_id_dict)  # key is boundary edge, value is adjacent x in other blocks
    ...
    def get_argmax_fx_with_proj_parallel_2(self, XT, Omega_X):
        ...
        max_iter = 2000
        # tao = 50  # number of cpus
        # tao = 5  # number of cpus
        # tao = 50 #if self.num_blocks / 2 > 50 else self.num_blocks / 2 # number of cpus, todo !!!
        tao = 50 if self.num_blocks / 2 > 50 else self.num_blocks / 2 # number of cpus, todo !!!
        n = self.num_blocks
        theta = tao / float(n)
        ...
        for k in range(max_iter):
            ...
            for block_index in S:
                ...
                omega_x = Omega_X[block_index]
                indicator_x[list(omega_x)] = 1.0  # used for projection
                boundary_xs_dict = self.get_boundary_xs(Y, self.boundary_edges_dict[block_index],
                                                        self.nodes_id_dict)  # key is boundary edge, value is adjacent x in other blocks
                y = Y[nodes]
                z = Z_next[nodes]
                x = X[nodes]
            ...
            for block_index in return_dict.keys():
                ...
                Z_next[self.nodes_set[block_index]] = return_dict[block_index] # fixme, sort?
            # for (block_index, output) in block_outputs:
    ...
    def get_block_gradient(self, z, z_next, fea, y, boundary_xs_dict, theta, tao, n, v):
        grad = self.get_ems_grad(z_next, fea) # nabla \phi(z)
        grad -= self.get_penality_grad(y, boundary_xs_dict) # nabla f(y_k)
