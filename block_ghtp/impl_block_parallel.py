...
def optimize(instance, sparsity, trade_off, learning_rate, max_iter, epsilon=1e-3, logger=None, tao=None):
    ...
    acc_proj_time = 0.
    func = ParallelSumEMS(features=instance['features'], trade_off=trade_off, nodes_set=nodes_set, boundary_edges_dict=boundary_edges_dict, nodes_id_dict=nodes_id_dict, tao=tao)
    if logger:
    ...
    for iter in range(max_iter):
        ...
        for b in range(num_blocks):
            ...
            boundary_xs_dict = get_boundary_xs(current_x, boundary_edges_dict[b], nodes_id_dict)  # key is boundary edge, value is adjacent x in other blocks
        ...
        bx_array = func.argmax_obj_with_proj_parallel(current_x, omega_x_list) # solve argmax problem with block coordinate ascent
