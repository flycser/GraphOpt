...
# @Filename : block_wikivote_serial_exp_2
# @Date     : 07/29/2019 14:07:14
# @Poject   : GraphOpt
...
def run_wikivote():
    ...
    for case_id in range(10):
        data_dir = "/network/rit/lab/ceashpc/share_data/{}/run{}/blocks_{}_mu_{}_true_{}_case_{}.pkl".format(
            graph_type, run,  num_blocks, mu1, true_size, case_id)
        data = pickle.load(open(data_dir, "rb"))
        true_subgraph = data["true_subgraph"]
        G = data["graph"]
        print("true_subgraph", true_subgraph)

        for trade_off in [x * 0.00001 for x in range(1, 2)]:
            for sparsity in range(70, 71):
                # case_id = "mu_{}_true_{}_case_{}".format(mu1, true_size, case_id)
                # log_file =
# "/network/rit/lab/ceashpc/share_data/fei_results/tmp/semi/run{}/blocks_{}_mu_{}_true_{}_case_{}.out".format(
#                     run, num_blocks, mu1, true_size, case_id)
#                 with open(log_file, "w") as f:
#                     f.write("trade_off= {}, sparsity={}\n".format(trade_off, sparsity))
                log_file = None

                result_file = "/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_wikivote_2/serial_{}_{}.out".format(sparsity, trade_off)
