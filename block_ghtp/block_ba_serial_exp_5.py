...
def run_edge():
    ...
    for trade_off in [x * 0.01 for x in range(1, 2)]:
        for sparsity in spar_list:
            log_file = None
            result_file = "/network/rit/lab/ceashpc/share_data/GraphOpt/log/ghtp/block_ba_6/nodes_{}_blocks_{}_mu_{}_subsize_{}_{}_deg_{}_{}_{}_serial_2.out".format(
                num_nodes, num_blocks, mu1, true_size, true_size, deg, data_type, case_id)
            para = (data, data_type, case_id, func_name, max_iter, trade_off, sparsity, log_file, result_file)
            input_paras.append(para)
            worker(para)
