...
def relabel_nodes(node_sets):
    ...
    node_id_dict = {}
    for block_nodes in node_sets:
        ind = 0
        for node in sorted(block_nodes):
            node_id_dict[node] = ind
            ind +=1

    return node_id_dict

def relabel_edges(graph, nodes_set, node_id_dict):
    """
    ...
    for block_nodes in nodes_set:
        ...
        for u, v in nx.subgraph(graph, block_nodes).edges():
            edges.append((node_id_dict[u], node_id_dict[v]))

        relabeled_edges_set.append(edges)

    return np.array(relabeled_edges_set)

def post_process_block(graph, raw_pred_sugraph, dataset=None):

    subgraph = nx.subgraph(graph, raw_pred_sugraph)
...
def get_boundary_xs(x, block_boundary_edges, node_id_dict):
    boundary_xs_dict = {}
    for (u, v) in block_boundary_edges:
        node_id_in_block = node_id_dict[u]
        adj_x_val = x[v]
...
if __name__ == '__main__':
    ...
    width = 10
    graph = nx.grid_2d_graph(width, width)
    mapping = {}
    for (i, j) in graph.nodes():
        mapping[(i, j)] = i * width + j
    graph = nx.relabel_nodes(graph, mapping)
    print(graph.nodes())
    raw_pred_subgraphs = [[1, 2, 3, 4, 5, 8, 17], [4, 5, 6, 7, 8, 16, 17, 33], [6, 7, 8, 11, 13]]
    refined_pre_subgraphs = post_process_evo(graph, raw_pred_subgraphs)
    for subgraph in refined_pre_subgraphs:
        print(subgraph)
