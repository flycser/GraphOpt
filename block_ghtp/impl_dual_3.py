...
# @Filename : impl_dual_3
# @Date     : 08/05/2019 18:08:03
# @Poject   : GraphOpt
...
def optimize(instance, sparsity, threshold, trade_off, learning_rate, max_iter, epsilon=1e-3, logger=None):
    ...
    for iter in range(max_iter):
        ...
        gamma_y = dense_projection(second_graph, norm_grad_y, threshold, min, maxx, step, start_node, lcc_diameter=2*lcc_diameter, normalize=True, sort=False)
        iter_proj_time += (time.time() - start_proj_time)
        ...
        start_proj_time = time.time()
        psi_y = dense_projection(second_graph, by, threshold, min, maxx, step, start_node, lcc_diameter=2*lcc_diameter, normalize=False, sort=False)
        iter_proj_time += (time.time() - start_proj_time)
...
def dense_projection(graph, weight, threshold, min, max, step, start_node, lcc_diameter, normalize=False, sort=False, times=2):
    ...
    for lmbd in range(min, max, step):
        ...
        for i in range(times):
            ...
            X = BFNS(graph, weight, lmbd, start_node, lcc_diameter, normalize=normalize, sort=sort, verbose=False)
