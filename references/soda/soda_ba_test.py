...
# @Filename : soda_ba_test
# @Date     : 07/22/2019 16:35:28
# @Poject   : GraphOpt
...
def soda_scan(V, edges, W, radius, s, C, true_nodes, g):
    ...
    for i,S in enumerate(S_data):
        ...
        #print(S)
        if len(get_neighbors(edges, S)) < 2000:
...
def test_on_ba(id):
    file_path = '/network/rit/lab/ceashpc/share_data/GraphOpt/DCS/soda/data/'
    file_list, keywords = get_data_file_list(file_path)
    index = 0
    for each_file in file_list:
        start_time = time.time()
        fp = open('/network/rit/lab/ceashpc/share_data/GraphOpt/DCS/soda/final_result_SODA_ba_{}.txt'.format(id), 'a')
        V, edges, W, true_nodes, true_features, g = load_ba(os.path.join(file_path, each_file))
        type_ = keywords[index]
        result = []
        best_fm = -1.0
        radius_list = [2, 3, 4, 5]
        print('radius ', radius_list)
        for ridus in radius_list:
